...
const uart_instance_t *g_shell_uart;

void cherrysh_uart_callback(uart_callback_args_t *p_args)
{
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;

    if (p_args->event == UART_EVENT_RX_CHAR) {
        chry_ringbuffer_write_byte(&shell_rb, (uint8_t)p_args->data);
    }
...
static uint16_t csh_sput_cb(chry_readline_t *rl, const void *data, uint16_t size)
{
    ...
    {
        sci_b_uart_instance_ctrl_t *ctrl = (sci_b_uart_instance_ctrl_t *)g_shell_uart->p_ctrl;
        ctrl->p_reg->TDR_BY = ((uint8_t*)data)[i];
...
int shell_init(const uart_instance_t *shell_uart, bool need_login)
{
    chry_shell_init_t csh_init;

    g_shell_uart = shell_uart;
